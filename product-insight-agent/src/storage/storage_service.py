...
class StorageService:
    ...
    def _ensure_schema_exists(self):
        ...
        if (
            ...
            try:
                ...
                logger.warning(
                    f"Could not create schema {settings.database_schema}: {e}"
                )
            ...
            with self.SessionLocal() as session:
                for item in feedback_items:
                    ...
                    if not existing:
                        ...
                        logger.debug(
                            f"Feedback item {item.source}:{item.source_id} already exists"
                        )
        ...
        try:
            with self.SessionLocal() as session:
                for analysis in analyses:
                    ...
                    existing = (
                        session.query(InsightAnalysisDB)
                        .filter_by(feedback_id=analysis.feedback_id)
                        .first()
                    )
        ...
        try:
            with self.SessionLocal() as session:
                ...
                existing = (
                    ...
                    .filter_by(date=summary.date.date())
                    .first()
                )
    ...
    def get_feedback_items_by_date_range(
        ...
        """
        logger.info(
            f"Retrieving feedback items from {start_date.date()} to {end_date.date()}"
        )
    ...
    def get_analyses_by_date_range(
        ...
        """
        logger.info(
            f"Retrieving analyses from {start_date.date()} to {end_date.date()}"
        )
    ...
    def get_feedback_item_by_id(self, feedback_id: UUID) -> Optional[FeedbackItem]:
        ...
        try:
            with self.SessionLocal() as session:
                db_item = (
                    session.query(FeedbackItemDB).filter_by(id=feedback_id).first()
                )
    ...
    def get_statistics(self) -> dict:
        ...
        try:
            with self.SessionLocal() as session:
                ...
                # Get date ranges
                earliest_feedback = (
                    session.query(FeedbackItemDB.timestamp)
                    .order_by(FeedbackItemDB.timestamp.asc())
                    .first()
                )
                latest_feedback = (
                    session.query(FeedbackItemDB.timestamp)
                    .order_by(FeedbackItemDB.timestamp.desc())
                    .first()
                )
                ...
                return {
                    ...
                    "earliest_feedback": (
                        earliest_feedback[0] if earliest_feedback else None
                    ),
                    "latest_feedback": latest_feedback[0] if latest_feedback else None,
                    "analysis_coverage": (
                        (analysis_count / feedback_count * 100)
                        if feedback_count > 0
                        else 0
                    ),
                }
    ...
    def get_goal_proposals(
        self, status: Optional["GoalProposalStatus"] = None, limit: int = 100
    ) -> List[GoalProposal]:
        """
    ...
    def get_goal_proposal_by_id(self, proposal_id: UUID) -> Optional[GoalProposal]:
        ...
        try:
            with self.SessionLocal() as session:
                db_proposal = (
                    session.query(GoalProposalDB)
                    .filter(GoalProposalDB.id == proposal_id)
                    .first()
                )
    ...
    def update_goal_proposal_status(
        self, proposal_id: UUID, new_status: "GoalProposalStatus"
    ) -> bool:
        """
        ...
        try:
            with self.SessionLocal() as session:
                ...
                if db_proposal:
                    ...
                    session.commit()
                    logger.info(
                        f"Updated goal proposal {proposal_id} status to {new_status}"
                    )
                    return True
                else:
    ...
    def get_recent_goal_proposals(self, days: int = 30) -> List[GoalProposal]:
        ...
        try:
            ...
            with self.SessionLocal() as session:
                db_proposals = (
                    session.query(GoalProposalDB)
                    .filter(GoalProposalDB.created_at >= cutoff_date)
                    .order_by(GoalProposalDB.created_at.desc())
                    .all()
                )
            ...
            logger.info(
                f"Retrieved {len(proposals)} goal proposals from last {days} days"
            )
            return proposals
