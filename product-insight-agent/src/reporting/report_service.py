...
class ReportingService:
    ...
    def _calculate_sentiment_breakdown(
        self, analyses: List[InsightAnalysis]
        ...
        sentiment_counts = Counter(
            analysis.sentiment_analysis.sentiment for analysis in analyses
        )
    ...
    def _calculate_topic_breakdown(
        ...
        return {topic: topic_counts.get(topic, 0) for topic in TopicTag}

    def _extract_key_insights(
        self, feedback_items: List[FeedbackItem], analyses: List[InsightAnalysis]
    ) -> List[str]:
        """
        ...
        if total_feedback > 0:
            positive_pct = (
                sentiment_breakdown[SentimentType.POSITIVE] / total_feedback
            ) * 100
            negative_pct = (
                sentiment_breakdown[SentimentType.NEGATIVE] / total_feedback
            ) * 100

            if positive_pct > 60:
                insights.append(
                    f"Predominantly positive feedback ({positive_pct:.1f}% positive)"
                )
            elif negative_pct > 60:
                insights.append(
                    f"High volume of negative feedback ({negative_pct:.1f}% negative)"
                )
            else:
        ...
        # Topic insights
        topic_breakdown = self._calculate_topic_breakdown(analyses)
        top_topics = sorted(topic_breakdown.items(), key=lambda x: x[1], reverse=True)[
            :3
        ]
        ...
        if top_topics[0][1] > 0:
            ...
            insights.append(
                f"Primary discussion topic: {top_topic_name} ({top_topics[0][1]} mentions)"
            )
        ...
        if len(sources) > 1:
            insights.append(
                f"Feedback collected from {len(sources)} sources: {', '.join(sources)}"
            )
        ...
        try:
            ...
            for i, item in enumerate(feedback_items[:5]):  # First 5 items
                sample_feedback.append(
                    f"{i+1}. {item.title or 'No title'}: {item.content[:200]}..."
                )
            ...
            topic_stats = "\n".join(
                [
                    ...
                    for topic, count in sorted(
                        topic_breakdown.items(), key=lambda x: x[1], reverse=True
                    )
                    if count > 0
                ]
            )
...
The primary topic of discussion was {top_topic_name}, mentioned in {top_topic[1]} feedback items. """
    ...
    def generate_trend_analysis(
        self, summaries: List[DailySummary], days: int = 7
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "average_daily_feedback": sum(total_feedback_trend)
            / len(total_feedback_trend),
            "most_active_day": summaries[
                total_feedback_trend.index(max(total_feedback_trend))
            ].date.isoformat(),
            "overall_sentiment": {
    ...
    def detect_significant_trends(
        ...
        sentiment_trends = self._detect_sentiment_shifts(
            summaries, feedback_items, analyses
        )
        ...
        recurring_trends = self._detect_recurring_issues(
            summaries, feedback_items, analyses
        )
        ...
        logger.info(
            f"Detected {len(significant_trends)} significant trends from {len(trends)} total trends"
        )
    ...
    def _detect_sentiment_shifts(
        ...
        recent_negative = sum(
            s.sentiment_breakdown.get(SentimentType.NEGATIVE, 0)
        ...
        if recent_total > 0 and historical_total > 0:
            ...
            if (
                recent_negative_ratio > historical_negative_ratio + 0.2
                and recent_negative >= 5
            ):
                ...
                recent_feedback = [
                    ...
                    if any(
                        analysis.feedback_id == item.id
                        and analysis.sentiment_analysis.sentiment
                ...
                if recent_feedback:
                    ...
                    sentiment_dist = {
                        ...
                        SentimentType.POSITIVE: sum(
                            s.sentiment_breakdown.get(SentimentType.POSITIVE, 0)
                            for s in recent_summaries
                        ),
                        SentimentType.NEUTRAL: sum(
                            s.sentiment_breakdown.get(SentimentType.NEUTRAL, 0)
                            for s in recent_summaries
                        ),
                    }
                    ...
                    trends.append(
                        TrendAnalysis(
                            ...
                            confidence=min(
                                0.9,
                                (recent_negative_ratio - historical_negative_ratio) * 3,
                            ),
                            ...
                            severity_score=min(
                                1.0,
                                (recent_negative_ratio - historical_negative_ratio) * 2
                                + (recent_negative / 20),
                            ),
                        )
                    )
    ...
    def _detect_topic_clusters(
        ...
        for topic, count in topic_counts.items():
            if (
                count >= 5 and count / total_feedback >= 0.15
            ):  # At least 5 items and 15% of total
                # Calculate sentiment distribution for this topic
                topic_analyses = [
                    a for a in analyses if topic in a.topic_analysis.topics
                ]
                sentiment_dist = {
                    SentimentType.POSITIVE: len(
                        [
                            a
                            for a in topic_analyses
                            if a.sentiment_analysis.sentiment == SentimentType.POSITIVE
                        ]
                    ),
                    SentimentType.NEGATIVE: len(
                        [
                            a
                            for a in topic_analyses
                            if a.sentiment_analysis.sentiment == SentimentType.NEGATIVE
                        ]
                    ),
                    SentimentType.NEUTRAL: len(
                        [
                            a
                            for a in topic_analyses
                            if a.sentiment_analysis.sentiment == SentimentType.NEUTRAL
                        ]
                    ),
                }
                ...
                sentiment_score = (
                    negative_ratio
                    if topic != TopicTag.GENERAL
                    else negative_ratio * 0.8
                )
    ...
    def _detect_volume_spikes(
        self, summaries: List[DailySummary], feedback_items: List[FeedbackItem]
    ) -> List["TrendAnalysis"]:
        ...
        if recent_volume > avg_volume * 2 and recent_volume >= 10:
            ...
            severity_score = min(
                1.0, (recent_volume / avg_volume) / 5
            )  # Normalize spike ratio
            ...
            trends.append(
                TrendAnalysis(
                    ...
                    primary_topics=[
                        TopicTag.GENERAL
                    ],  # Would need analysis to determine specific topics
                    sentiment_distribution=sentiment_dist,
                    key_indicators=[
        ...
        for summary in summaries:
            ...
            for topic, count in summary.topic_breakdown.items():
                if count > 0:
                    ...
                    negative_sentiment_count = summary.sentiment_breakdown.get(
                        SentimentType.NEGATIVE, 0
                    )
                    topic_negative_estimate = min(
                        count,
                        int(
                            negative_sentiment_count
                            * (count / sum(summary.topic_breakdown.values()))
                        ),
                    )
