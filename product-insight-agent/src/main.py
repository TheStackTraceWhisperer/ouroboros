    ...
    def ingest_data(self) -> List[FeedbackItem]:
        ...
        try:
            ...
            raw_feedback = self.reddit_connector.ingest_posts_since(
                since=since, limit=settings.max_posts_per_run
            )
            ...
            processed_feedback = self.data_processor.process_feedback_items(
                raw_feedback
            )
            ...
            if processed_feedback:
                ...
                logger.info(
                    f"Ingested and stored {len(processed_feedback)} feedback items"
                )
    ...
    def generate_goals_from_trends(self, days: int = 7) -> List[GoalProposal]:
        ...
        try:
            ...
            if goals:
                ...
                if success:
                    logger.info(
                        f"Successfully generated and stored {len(goals)} goal proposals"
                    )
                else:
...
@cli.command()
@click.option(
    "--date", type=click.DateTime(["%Y-%m-%d"]), help="Date for report (YYYY-MM-DD)"
)
def report(date):
    """Generate daily summary report."""
    agent = ProductInsightAgent()
...
def generate_goals(days):
    ...
    if goals:
        ...
        for i, goal in enumerate(goals, 1):
            ...
            print(
                f"   Trend: {goal.source_trend.trend_type.value.replace('_', ' ').title()}"
            )
...
@click.option(
    "--status",
    type=click.Choice(["pending", "approved", "rejected", "in_progress", "completed"]),
