...
from ..models import (
    TrendAnalysis, GoalProposal, GoalProposalStatus, FeedbackItem,
    InsightAnalysis, TrendType, TopicTag, SentimentType
)
...
class GoalGenerationService:
    ...
    def generate_goals_from_trends(self,
                                 trends: List[TrendAnalysis],
                                 supporting_feedback: List[FeedbackItem],
                                 supporting_analyses: List[InsightAnalysis]) -> List[GoalProposal]:
        """
        ...
        """
        logger.info(f"Generating goals from {len(trends)} trends with {len(supporting_feedback)} supporting feedback items")
        
        goals = []
        
        for trend in trends:
            try:
                goal = self.generate_single_goal(trend, supporting_feedback, supporting_analyses)
                if goal:
                    goals.append(goal)
            except Exception as e:
                logger.warning(f"Failed to generate goal for trend {trend.trend_type}: {e}")
                continue
        
        logger.info(f"Successfully generated {len(goals)} goal proposals")
        return goals
    
    def generate_single_goal(self,
                           trend: TrendAnalysis,
                           supporting_feedback: List[FeedbackItem],
                           supporting_analyses: List[InsightAnalysis]) -> Optional[GoalProposal]:
        """
    ...
    def _filter_relevant_feedback(self,
                                trend: TrendAnalysis,
                                supporting_feedback: List[FeedbackItem],
                                supporting_analyses: List[InsightAnalysis]) -> List[FeedbackItem]:
        """Filter feedback items relevant to the given trend."""
        relevant_items = []
        
        # Create a mapping of feedback ID to analysis
        analysis_map = {analysis.feedback_id: analysis for analysis in supporting_analyses}
    ...
    def _generate_goal_with_llm(self,
                              trend: TrendAnalysis,
                              relevant_feedback: List[FeedbackItem]) -> Optional[dict]:
...
"""
        ...
        try:
            ...
            if len(goal_data["description"]) < 50:
                logger.warning(f"Goal description too short: {len(goal_data['description'])}")
                return None
    ...
    def _prepare_llm_context(self,
                           trend: TrendAnalysis,
                           relevant_feedback: List[FeedbackItem]) -> str:
...
"""
    ...
    def test_goal_generation(self, sample_trend: TrendAnalysis, sample_feedback: List[FeedbackItem]) -> bool:
        """
