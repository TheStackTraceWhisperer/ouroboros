...
class TestSentimentAnalysis:
    ...
    def test_sentiment_confidence_validation(self):
        ...
        # Valid confidence
        analysis = SentimentAnalysis(sentiment=SentimentType.NEUTRAL, confidence=0.5)
        assert analysis.confidence == 0.5

        # Invalid confidence should raise error
        with pytest.raises(ValueError):
            SentimentAnalysis(sentiment=SentimentType.POSITIVE, confidence=1.5)  # > 1.0
...
class TestTopicAnalysis:
    ...
    def test_empty_topics_list(self):
        ...
        analysis = TopicAnalysis(
            topics=[], confidence=0.1, reasoning="No clear topics identified."
        )
